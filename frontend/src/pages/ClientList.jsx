                ...
                <div className="p-6">
                    ...
                    <div className="flex justify-end space-x-3">
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg font-semibold transition duration-150 ${primaryButtonColor}`}
                        >
                            {isConfirm ? 'Delete' : 'OK'}
                        </button>
                    </div>
...
// ==========================================================
// Component A/C: ClientForm (Handles Add and Edit)
// ==========================================================

const ClientForm = ({ goBack, initialClientData = null }) => {
    // ⭐ Get the authentication token
    const { token } = useAuth(); 

    // Initial state setup: use initialClientData for editing, or empty object for adding
    const initialState = initialClientData || {
    ...
    const isEditing = !!initialClientData; 
    ...
    const closeModal = () => {
        ...
        // If successful in adding or editing, navigate back after closing the modal
    ...
    const handleSubmit = async (e) => {
        ...
        // ⭐ SECURITY CHECK: Deny submission if token is missing
        if (!token) { 
             setModal({
                 isOpen: true,
                 title: 'Authentication Required',
                 message: "You must be logged in to save client data.",
                 type: 'error'
             });
             setIsLoading(false);
             return;
        ...
        try {
            ...
            setModal({
                ...
                message: isEditing 
                    ? `Client "${savedClient.name}" updated successfully.`
                    : `Client "${savedClient.name}" added successfully.`,
                type: 'success'
            });

            // If adding a client, clear the form after success
            if (!isEditing) {
            ...
            setModal({
                isOpen: true,
                title: isEditing ? 'Failed to Update Client' : 'Failed to Save Client',
    ...
    const renderInput = (label, name, type = 'text') => (
        <div className="mb-4">
            ...
            <input
                ...
                id={name}
                name={name}
                value={formData[name] || ''} // Ensure value is a controlled component
                onChange={handleChange}
                required={['name', 'email', 'businessNumber'].includes(name)} 
        ...
        <div className="bg-gray-100 min-h-screen p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-4xl mx-auto my-8">
                <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-3">
                    {isEditing ? `Edit Client: ${initialClientData.name}` : 'Add New Client'}
                </h2>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end space-x-4 mt-8 pt-4 border-t">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                        >
                            {isLoading ? (isEditing ? 'Updating...' : 'Saving...') : (isEditing ? 'Update Client' : 'Save Client')}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <CustomModal
                ...
                onClose={closeModal} // This now also handles navigation back on success
                type={modal.type === 'success' ? 'alert' : 'error'} // Use 'alert' for success message display
            />
        </div>
    );
};
...
const ClientList = () => {
    ...
    const fetchClients = useCallback(async () => {
        // ⭐ SECURITY CHECK: Prevent fetching if not authenticated
        if (!token) {
            setError("Access Denied. Please log in to view clients.");
            setClients([]); 
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorData = await response.json();
                throw new Error(errorData.message || `Failed to fetch clients: ${response.statusText}`);
            }
    ...
    // Fetch data on component mount and after navigation actions (add/edit/delete)
    useEffect(() => {
        if (view === 'list') {
            fetchClients();
        }
    ...
    const confirmDelete = async () => {
        ...
        // ⭐ SECURITY CHECK: Deny deletion if token is missing
        if (!token) {
            setDeleteModal({ isOpen: false, clientToDelete: null });
            setMessageModal({
                isOpen: true,
                title: 'Access Denied',
                message: `You must be logged in to delete client data.`,
                type: 'error'
            });
            return;
        }
        ...
        try {
            ...
            // Remove the client from the local state immediately for a fast UI update
        ...
        return (
            <div className="flex justify-center items-center h-screen bg-gray-50">
                <div className="text-center p-8 bg-white rounded-xl shadow-xl">
                    <h3 className="text-2xl font-bold text-red-600 mb-2">Access Restricted 🔒</h3>
                    <p className="text-gray-700">Please log in to view and manage client records.</p>
                </div>
            </div>
        );
    }
    ...
    if (view === 'add') {
        return <ClientForm goBack={handleGoBack} />;
    }
    
    if (view === 'edit' && editClientData) {
        return <ClientForm goBack={handleGoBack} initialClientData={editClientData} />;
    }

    // Default loading state for list view

...
const AuthForm = ({ onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                if (isSignIn) {
                    // Call login function to store token and user data
                    login(result.token, result.user); 
                    console.log('Login successful. User:', result.user.username);
                } else {
                    console.log('Registration successful. Switching to Sign In.');
                    // After successful signup, switch to the login view
                    setIsSignIn(true);
                }
                
                // 🔑 Call the onSuccess prop to close the modal!
                if (onSuccess) {
                    onSuccess();
                }
    ...
    return (
        // The form styling is simple, allowing the parent Modal component to handle positioning.
